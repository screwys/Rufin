...
pub fn folder_target(uri: &str) -> Result<FolderTarget, String> {
    ...
    if matches!(url.scheme(), "http" | "https" | "smb") {
        ...
        return Ok(FolderTarget::Finder(uri.to_owned()));
    }
    ...
    Ok(FolderTarget::File(file))
}

#[cfg(target_os = "macos")]
pub async fn open_in_finder(uri: &str) -> Result<(), String> {
...
fn windows_network_path(url: &Url) -> Result<Option<String>, String> {
        ...
        .filter(|part| !part.is_empty())
    {
        path.push('\\');
        path.push_str(
            &glib::Uri::unescape_segment(Some(part), None, None)
