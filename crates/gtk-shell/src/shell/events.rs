...
pub(crate) fn install_product_event_receivers(shell: &Rc<Shell>, receivers: ProductReceivers) {
    ...
    glib::spawn_future_local(async move {
        while let Ok(frame) = visualizer.recv().await {
            ...
            if matches {
                event_shell.player_ui.apply_visualizer_levels(frame.levels);
            }
        }
    });
    ...
    glib::spawn_future_local(async move {
        while let Ok(update) = release_updates.recv().await {
            apply_release_update(&event_shell.preferences, update);
        }
    });
}
...
fn apply_source_event(shell: &Rc<Shell>, event: SourceEvent) {
    match event {
        ...
        SourceEvent::Selected {
            ...
            apply_selected_source(shell, configured, selected);
        }
        ...
        SourceEvent::PlaylistSourceMismatch(source_id) => {
            let source_name = shell
                ...
                .map(gtk_widgets::source_labels::configured_source_display_name)
...
fn apply_configured_sources(
    shell: &Rc<Shell>,
    configured: rufin_core::runtime::source::ConfiguredSources,
) {
...
fn finish_source_assignment(
    ...
    let rebuild_sidebar = source_changed || library_changed || scope_changed;
    ...
    if source_changed {
        ...
        shell.artwork.reset_cover_pipeline_state();
        if matches!(
            shell.navigation.routes.borrow().current(),
            Route::History | Route::Playlists | Route::SmartPlaylists
        ) {
            shell.replace_current_route_when_ready();
        } else {
            shell.reset_navigation_to_home();
        }
    } else if library_changed || scope_changed {
    ...
    if !shell.startup.route_revealed.get() && !shell.source.operation.borrow().blocks_library() {
...
fn apply_selected_library_replacement(
    ...
    let scope_changed = shell.selected_library().is_none_or(|previous| {
        previous.music_folder_key != selected.music_folder_key
            || previous.music_folder_object_id != selected.music_folder_object_id
    });
...
fn apply_catalog_publication(shell: &Rc<Shell>, publication: CatalogPublication) {
    ...
    if let Some(favorite) = publication.favorite {
        ...
        if favorite.requested != favorite.effective {
            shell
                .control_feedback
                .show_feedback_toast(tr("Could not update favorites"));
        }
    ...
    match publication.change {
        ...
        CatalogChange::Playlists(playlist) => {
            refresh_context_playlist_picker(shell);
            super::navigation::refresh_sidebar_pins(shell);
            if current == Route::Playlists
                || matches!(current, Route::PlaylistDetail(key) if playlist.is_none_or(|playlist| playlist == key))
            {
                shell.refresh_mounted_catalog();
            }
        }
...
fn apply_source_notice(shell: &Rc<Shell>, notice: SourceNotice) {
    let matches_selected = shell
        .selected_library()
        .as_deref()
        .is_some_and(|selected| selected.source_key == notice.source_key);
    if !matches_selected {
        return;
    }
    shell
        .control_feedback
        .show_feedback_toast(match notice.kind {
    ...
    *shell.source.operation.borrow_mut() = operation.clone();
    ...
    match &operation {
        SourceOperation::Adding { .. } | SourceOperation::Switching { .. } => {
            if started_blocking {
                shell.preferences.close_preferences_dialog();
            }
        ...
        SourceOperation::Failed {
            ...
            if *add_form {
                if !shell.preferences.restore_add_server_dialog_after_failure() {
                    shell.preferences.update_add_server_dialog();
                }
                if previously_blocked {
                    shell.schedule_startup_route_reveal();
                }
        ...
        SourceOperation::Idle => {
            if completed_add {
                shell.preferences.complete_add_server_dialog();
            }
            shell.preferences.update_add_server_dialog();
            if shell.selected_library().is_some() && !shell.startup.route_revealed.get() {
                shell.schedule_startup_route_reveal();
            }
        }
    }
...
fn apply_source_discovery(shell: &Rc<Shell>, update: DiscoveryUpdate) {
    ...
    *shell.source.discovery_status.borrow_mut() = update.status.clone();
    ...
    shell.preferences.update_add_server_discovery();
}

fn apply_playback_projection(shell: &Rc<Shell>, projection: playback::PlaybackProjection) {
    let previous_player = shell.player_ui.selected_playback().as_deref().cloned();
    ...
    shell.player_ui.replace_player(view.clone());
...
fn finish_playback_projection(
    ...
    if queue_window_changed && shell.player_ui.selected_queue().is_some() {
        shell.player_ui.refresh_queue_window();
    }
    ...
    if position_only {
        ...
        shell.player_ui.update_bottom_player_transport();
    }
    ...
    if media_changed {
        ...
        shell.player_ui.cancel_scheduled_lyrics_highlight();
    ...
    if let Some(now_playing) = now_playing.as_ref() {
        shell.player_ui.update_fullscreen_player_with(now_playing);
    }
    if fullscreen_refresh != FullscreenPlaybackRefresh::None {
        shell.player_ui.sync_visualizer_state();
    }
    if lyrics_timing_changed {
        shell.player_ui.update_lyrics_highlight();
    }
    ...
    if queue_panel_changed && !queue_window_changed {
        shell.player_ui.schedule_queue_panel_render();
    }
}
...
fn show_local_folder_recovery(shell: &Rc<Shell>, folder: String) {
    ...
    toast.connect_button_clicked(move |toast| {
        toast.dismiss();
        crate::preferences::locate_local_folder(&recovery_shell.preferences, folder.clone());
    });
...
fn apply_waveform(shell: &Rc<Shell>, waveform: WaveformProjection) {
    shell.player_ui.set_waveform(waveform);
    shell.player_ui.update_bottom_player_transport();
}

fn apply_lyrics_event(shell: &Rc<Shell>, event: rufin_core::lyrics::LyricsEvent) {
    match event {
        rufin_core::lyrics::LyricsEvent::JapaneseDictionaryChanged(status) => {
            shell.player_ui.apply_japanese_dictionary_status(status)
        }
        rufin_core::lyrics::LyricsEvent::Current(projection) => {
            shell.player_ui.apply_current_lyrics(projection)
        }
        rufin_core::lyrics::LyricsEvent::SearchFinished {
            media_id,
            query,
            result,
        } => match result {
            Ok(results) => shell.player_ui.apply_lyrics_search_results(
                media_id,
                query.artist_name,
                query.track_name,
                results,
            ),
            Err(error) => shell.player_ui.apply_lyrics_search_failed(
                media_id,
                query.artist_name,
                query.track_name,
                error,
        ...
        rufin_core::lyrics::LyricsEvent::Saved { media_id, path } => {
            if current_playback_media_id(shell.player_ui.selected_playback().as_deref()).as_ref()
                == Some(&media_id)
            {
                shell.player_ui.apply_lyrics_saved(media_id, path);
            }
        }
        rufin_core::lyrics::LyricsEvent::SourceSaveFailed { media_id, error } => {
            if current_playback_media_id(shell.player_ui.selected_playback().as_deref()).as_ref()
                == Some(&media_id)
            {
                shell.control_feedback.show_feedback_toast(match error {
