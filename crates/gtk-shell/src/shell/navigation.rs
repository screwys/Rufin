...
use gtk_media_menus::media_menus::{
...
use gtk_widgets::cover_controls::cover_play_only_hover_controls;
use rufin_core::playback::PlaybackTarget;

use crate::{SidebarPin, SidebarRouteItem};
use adw::prelude::*;
...
use gtk_widgets::library_fields::{playlist_artwork, smart_playlist_display_name};
use gtk_widgets::route::{CollectionCategory, Route};
...
use gtk_widgets::controls::{PLAY_ICON, PLAY_LATER_ICON, PLAY_NEXT_ICON, icon_button};
use gtk_widgets::mounted_route::route_current_track;
use localization::tr;
...
pub(crate) fn update_sidebar_pin_playback(shell: &Shell) {
    ...
    let playback_context_id = current
        .as_ref()
        .and_then(|current| current.context.as_ref())
...
fn update_native_route_selection(shell: &Shell, active_route: &Route, pin_is_selected: bool) {
    ...
    let selected_index = if pin_is_selected {
        ...
        items
            .clone()
            .position(|entry| {
                gtk_widgets::route::sidebar_route_descriptor(entry.item).root_route == *active_route
            })
...
fn update_button_navigation_selection(container: &gtk::Box, active_route: &Route) {
    ...
    while let Some(widget) = child {
        ...
        let descriptor = gtk_widgets::route::sidebar_route_descriptor(item);
...
struct SidebarPinIdentity {
    smart_context: Option<(library::SourceKey, Option<library::FolderKey>)>,
    pins: Vec<SidebarPin>,
}

impl SidebarPinItem {
    fn stored_pin(&self) -> SidebarPin {
        match self {
            Self::Album(album) => SidebarPin::Album {
                source_id: library::source_entity_parts(&album.media_uri)
                    .expect("catalog entity has a source identity")
                    .0,
                album_id: album.object_id.clone(),
            },
            Self::Artist(artist, album_artist) => SidebarPin::Artist {
                source_id: library::source_entity_parts(&artist.media_uri)
                    .expect("catalog entity has a source identity")
                    .0,
    ...
    fn route(&self) -> Route {
        match self {
            Self::Album(album) => Route::AlbumDetail(album.media_uri.clone()),
            Self::Artist(artist, album_artist) => {
                gtk_library::artist_detail_route(artist.media_uri.clone(), *album_artist)
            }
    ...
    fn playback_target(&self) -> PlaybackTarget {
        match self {
            Self::Album(album) => PlaybackTarget::Album(album.media_uri.clone()),
            Self::Artist(artist, album_artist) => {
                if *album_artist {
        ...
        match self {
            ...
            Self::Playlist(playlist) => {
                present_playlist_context_menu(
                    ...
                    None,
                    position,
                    gtk_player::state::current_playback_track(
...
pub(crate) fn request_sidebar_pins(shell: &Rc<Shell>) {
    ...
    let identity = SidebarPinIdentity {
        smart_context: pins
            .iter()
            .any(|pin| matches!(pin, SidebarPin::SmartPlaylist { .. }))
            .then(|| {
                selected
                    .as_ref()
                    .map(|selected| (selected.source_key, selected.music_folder_key))
            })
            .flatten(),
        pins: pins.clone(),
    };
        ...
        .replace(Some(identity.clone()));
    {
        let retained = {
            ...
            items.retain(|item| pins.contains(&item.stored_pin()));
            items.clone()
        };
    ...
    let current_source = selected.as_ref().map(|selected| selected.source_key);
    let folder = selected
        .as_ref()
        ...
        for pin in pins {
            let source = match &pin {
                ...
                | SidebarPin::Genre { source_id, .. } => {
                    database.source_identity_key(source_id).await?
                }
                _ => None,
            };
            let item = match pin {
                SidebarPin::Album { album_id, .. } => {
                    ...
                    database
                        .album_rows(source, &[key], None, &cancellation)
                        .await?
                        .pop()
                ...
                SidebarPin::Artist {
                    ...
                    database
                        .artist_rows(source, &[key], album_artist, None, &cancellation)
                        .await?
                        .pop()
                ...
                SidebarPin::Genre { genre_id, .. } => {
                    ...
                    database
                        .genre_rows(source, &[key], None, &cancellation)
                        .await?
                        .pop()
                ...
                SidebarPin::SmartPlaylist { playlist_id, .. } => {
                    ...
                    database
                        .smart_playlist_rows(current_source, &[key], folder, now, &cancellation)
                        .await?
                        .pop()
...
fn reorder_current_sidebar_pin_projection(
    ...
    let desired = shell.settings.current.borrow().sidebar.pins.clone();
    let (moved_key, target_key) = {
        let mut items = shell.navigation.pin_items.borrow_mut();
        let Some(moved_index) = items.iter().position(|item| item.stored_pin() == *moved) else {
            return;
        };
        let moved_item = items.remove(moved_index);
        let Some(target_index) = items.iter().position(|item| item.stored_pin() == *target) else {
            return;
        };
...
fn reconcile_sidebar_pin_widgets(shell: &Rc<Shell>, items: &[SidebarPinItem]) {
    ...
    let previous_items = shell
        ...
        .borrow()
        .iter()
        .filter_map(|item| sidebar_pin_route_key(&item.route()).map(|key| (key, item.clone())))
        .collect::<HashMap<_, _>>();
...
fn sidebar_pin_row(shell: &Rc<Shell>, pin: SidebarPinItem) -> gtk::Overlay {
    ...
    let source_label = {
        ...
        let source_id = match &pin {
            SidebarPinItem::Playlist(playlist) => Some(playlist.source_id.as_deref()),
            SidebarPinItem::SmartPlaylist(playlist) => Some(
                configured
                    .selected_source_id
                    .as_ref()
                    .filter(|_| playlist.definition.current)
                    .map(sources::SourceId::as_str),
            ),
            _ => None,
        };
        source_id.map(|source_id| {
            gtk_widgets::source_labels::source_display_label(
                configured
                    .sources
                    .iter()
    ...
    if let Some((icon, name)) = source_label {
        let source = gtk::Image::from_icon_name(icon);
        source.set_pixel_size(gtk_widgets::source_labels::source_icon_pixel_size(icon, 12));
...
fn sidebar_pin_drag_variant(pin: &SidebarPin) -> glib::Variant {
    let (kind, source_id, object_id, album_artist) = match pin {
        SidebarPin::Album {
            source_id,
            album_id,
        } => (0_u8, Some(source_id), album_id, false),
        SidebarPin::Artist {
            source_id,
            artist_id,
            album_artist,
        } => (1, Some(source_id), artist_id, *album_artist),
        SidebarPin::Genre {
            source_id,
            genre_id,
        } => (2, Some(source_id), genre_id, false),
        SidebarPin::Playlist {
            source_id,
            playlist_id,
        } => (3, source_id.as_ref(), playlist_id, false),
        SidebarPin::SmartPlaylist { playlist_id } => (4, None, playlist_id, false),
    };
    (
        kind,
        source_id.map(ToString::to_string),
        object_id.clone(),
        album_artist,
    )
        .to_variant()
...
fn sidebar_pin_from_drag_value(value: &glib::Value) -> Option<SidebarPin> {
    ...
    let (kind, source_id, object_id, album_artist) =
        variant.get::<(u8, Option<String>, String, bool)>()?;
    let source_id = source_id.map(sources::SourceId::new);
    match kind {
...
fn install_playlist_pin_drop(
    ...
    drop_target.connect_drop(move |_, value, _, _| {
        ...
        gtk_media_menus::media_menus::add_drag_to_playlist(
            &shell.media_menus,
            playlist.clone(),
            source,
        )
...
fn sidebar_route_at_position_in(
    ...
    Some(gtk_widgets::route::sidebar_route_descriptor(item.item).root_route)
}
...
mod tests {
    ...
    fn sidebar_position_uses_visible_configured_route_order() {
        let mut items = rufin_core::settings::SidebarRouteItem::all()
            .into_iter()
    ...
    fn descriptor_ids_preserve_sidebar_settings_serialization() {
        for item in SidebarRouteItem::all() {
            ...
            assert_eq!(
                value.as_str(),
                Some(gtk_widgets::route::sidebar_route_descriptor(item).stable_id)
            );
            assert_eq!(
    ...
    fn nested_routes_leave_root_sidebar_items_unselected() {
        let routes = [
            Route::AlbumDetail("rufin:album".to_string()),
            Route::ArtistDetail("rufin:artist".to_string()),
            Route::AlbumArtistTracks("rufin:artist".to_string()),
            Route::GenreDetail(library::GenreKey::from_raw(1)),
            Route::MoodDetail(library::MoodKey::from_raw(1)),
            Route::Folders {
                path: vec![gtk_widgets::route::FolderPathItem {
                    id: String::new(),
                    name: String::new(),
                }],
            },
        ...
        for route in routes {
            assert!(SidebarRouteItem::all().into_iter().all(|item| {
                gtk_widgets::route::sidebar_route_descriptor(item).root_route != route
            }));
        }
    }
