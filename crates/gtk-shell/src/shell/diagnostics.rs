...
pub(super) fn present_diagnostics(shell: &Rc<Shell>) {
    ...
    let dialog: adw::Dialog =
        gtk_widgets::ui_resource::object(&builder, resource, "diagnostics_dialog");
    ...
    let debug: adw::SwitchRow = gtk_widgets::ui_resource::object(&builder, resource, "debug");
    debug.set_active(shell.diagnostics.debug_enabled());

    let log: gtk::TextView = gtk_widgets::ui_resource::object(&builder, resource, "log");
    log.update_property(&[gtk::accessible::Property::Label(&tr("Diagnostic log"))]);
    let scroller: gtk::ScrolledWindow =
        gtk_widgets::ui_resource::object(&builder, resource, "log_scroller");
    let copy: gtk::Button = gtk_widgets::ui_resource::object(&builder, resource, "copy");
    let save: gtk::Button = gtk_widgets::ui_resource::object(&builder, resource, "save");
    let status: gtk::Label = gtk_widgets::ui_resource::object(&builder, resource, "status");
    drop(builder);
