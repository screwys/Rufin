...
use gtk_widgets::artwork::ArtworkTile;
...
impl PreviewRow {
    fn new(history: bool) -> Self {
        ...
        gtk_widgets::recycled_cells::install_playing_indicator(
            &body.imp().title,
            &*body.imp().title_row,
        );
        ...
        row.add_css_class(gtk_widgets::interactions::CONTEXT_MENU_HOVER_OWNER_CLASS);
        Self { row, body, cover }
    }
    ...
    fn set_playing(&self, current: bool, paused: bool) {
        ...
        gtk_widgets::recycled_cells::set_track_playing(imp.title.upcast_ref(), current, paused);
        gtk_widgets::recycled_cells::set_track_playing(self.row.upcast_ref(), current, paused);
        ...
        gtk_widgets::localization::bind_widget_tooltip(
            &*imp.play,
...
pub(super) struct SearchPopup {
    ...
    host: gtk::Overlay,
    session: Rc<gtk_library::SearchSession>,
    popover: gtk::Popover,
...
impl SearchPopup {
    pub fn new(shell: &Rc<Shell>, host: &gtk::Overlay) -> Rc<Self> {
        ...
        let popup = Rc::new(Self {
            ...
            previews: std::array::from_fn(|_| {
                (0..gtk_library::SEARCH_PREVIEW_LIMIT)
        ...
        for row in &self.history {
            ...
            row.body.imp().play.connect_clicked(move |button| {
                if let (Some(popup), Some(row)) = (weak.upgrade(), target.upgrade()) {
                    ...
                    {
                        gtk_library::SearchSession::play_recent(
                            &result,
                            &catalog,
        ...
        if !self.popover.is_visible() {
            ...
            if let Some(shell) = self.shell.upgrade() {
                self.refresh_playback(
                    gtk_widgets::mounted_route::route_current_track(
        ...
        for (row, action) in actions.iter() {
            ...
            let playing = current.is_some_and(|current| match action {
                ...
                SearchAction::Recent(result) => {
                    gtk_library::SearchSession::recent_matches(result, current)
                }
                _ => false,
            });
    ...
    fn catalog(&self) -> Option<Rc<gtk_library::CatalogUi>> {
        ...
        match action {
            Some(SearchAction::Recent(result)) => {
                if let Some(catalog) = self.catalog() {
                    gtk_library::SearchSession::activate_recent(&result, &catalog);
                }
            }
