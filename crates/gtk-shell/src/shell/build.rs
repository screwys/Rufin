use gtk_player::outputs::{default_audio_output_options, warm_audio_output_cache};
use gtk_player::right_panel::build_right_panel;
use gtk_player::right_panel::{apply_sidebar_media_visibility, connect_sidebar_controls};
use gtk_player::{
...
use gtk_player::lyrics::state::LyricsState;
use gtk_player::state::PlaybackState;
use gtk_preferences::preferences::PreferencesState;
...
pub async fn build(
    ...
    let shell_root_builder = gtk_widgets::ui_resource::builder(shell_root_resource);
    gtk_widgets::objects!(shell_root_builder, shell_root_resource, {
    ...
    let fresh_start_banner: adw::Banner = gtk_widgets::ui_resource::object(
        &shell_root_builder,
        shell_root_resource,
        "fresh_start_banner",
    );
    ...
    let downloads = gtk_widgets::downloads::DownloadsState::new(Rc::clone(&settings_state));
    ...
    let navigation_builder = gtk_widgets::ui_resource::builder(navigation_resource);
    gtk_widgets::objects!(navigation_builder, navigation_resource, {
    ...
    let shell = Rc::new_cyclic(|weak: &std::rc::Weak<Shell>| {
        ...
        let player_ui = Rc::new(gtk_player::PlayerUi::new(
            playback_state,
            lyrics_state,
            player_view,
            player_right_panel,
        ...
        let web_controller = Rc::new(web::Controller::new(products.clone()));
        let preferences = Rc::new(gtk_preferences::Preferences {
            state: preferences,
            settings: Rc::clone(&settings_state),
            source: Rc::clone(&source),
            products: products.clone(),
            player_ui: Rc::clone(&player_ui),
            downloads: Rc::clone(&downloads),
            artwork: Rc::clone(&artwork),
            appearance: Rc::clone(&appearance),
            control_feedback: Rc::clone(&control_feedback),
            web_controller: Rc::clone(&web_controller),
            window: chrome.window.clone(),
            toast_overlay: chrome.toast_overlay.clone(),
            app_root_overlay: chrome.app_root_overlay.clone(),
            effects: {
                let weak = weak.clone();
                Box::new(move |effect| {
                    if let Some(shell) = weak.upgrade() {
                        shell.apply_preferences_effect(effect);
                    }
                })
            },
        });
    ...
    shell.preferences.bind_controller_appearance();
    ...
    fresh_start_banner.connect_button_clicked(move |_| {
        if let Some(shell) = weak.upgrade() {
            crate::preferences::backup::import_dialog(&shell.preferences);
        }
    });
    ...
    gtk::glib::spawn_future_local(async move {
        while let Ok(storage) = secret_storage_fallbacks.recv().await {
            ...
            if let Some(row) = shell.preferences.state.secret_storage_row.upgrade() {
    ...
    check_for_release_update(&shell.preferences);
    ...
    if shell.source.configured.borrow().sources.is_empty() {
        if shell.chrome.window.is_mapped() {
            shell.preferences.present_onboarding();
        } else {
    ...
    schedule_periodic_release_checks(&shell.preferences);
