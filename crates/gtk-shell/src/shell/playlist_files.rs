...
impl Shell {
    ...
    pub(crate) fn export_playlist_dialog(self: &Rc<Self>, target: PlaylistExport, name: &str) {
        ...
        glib::spawn_future_local(async move {
            let resource = gtk_preferences::ui_resource::PLAYLIST_FILE_DIALOG_RESOURCE;
            let builder = gtk_widgets::ui_resource::builder(resource);
            gtk_widgets::objects!(builder, resource, { export_options: adw::AlertDialog, export_format: adw::ComboRow, export_path_mode: adw::ComboRow, export_link: adw::SwitchRow });
            ...
            gtk_widgets::popup::install_light_dismiss(&export_options);
            if export_options
                .choose_future(Some(&shell.chrome.window))
                .await
                != "export"
    ...
    pub(crate) fn export_activity_dialog(
        ...
        let shell = Rc::clone(self);
        glib::spawn_future_local(async move {
            let resource = gtk_preferences::ui_resource::BACKUP_DIALOG_RESOURCE;
            let dialog: gtk::FileDialog = gtk_widgets::ui_resource::object(
                &gtk_widgets::ui_resource::builder(resource),
                resource,
                "activity_chooser",
            );
...
fn playlist_chooser() -> gtk::FileDialog {
    let resource = gtk_preferences::ui_resource::PLAYLIST_FILE_DIALOG_RESOURCE;
    let builder = gtk_widgets::ui_resource::builder(resource);
    gtk_widgets::objects!(builder, resource, { chooser: gtk::FileDialog, playlist_filter: gtk::FileFilter });
...
async fn playlist_file_location(shell: &Rc<Shell>, filename: &str) -> Option<FileLocation> {
    ...
    let resource = gtk_preferences::ui_resource::PLAYLIST_FILE_DIALOG_RESOURCE;
    let builder = gtk_widgets::ui_resource::builder(resource);
    gtk_widgets::objects!(builder, resource, { dialog: adw::AlertDialog, path: adw::EntryRow });
