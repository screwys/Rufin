...
async fn local_component_larger_than_one_sql_batch_converges_atomically() {
    ...
    for path in &paths {
        write_silent_wav(path, 3).expect("edit WAV for restart catch-up");
    }
    let caught_up = publication(
        source
            .catch_up_local(
                &database,
                changed.source,
            ...
            .await
            .expect("paged restart catch-up"),
    );
    assert_eq!(caught_up.catalog_revision, changed.catalog_revision + 1);
}
