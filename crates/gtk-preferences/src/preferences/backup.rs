...
use adw::prelude::*;
use backup::{BackupContents, BackupFrequency};
use gtk::{gio, glib};
...
fn content_rows(builder: &gtk::Builder, mut selected: BackupContents) -> [adw::SwitchRow; 7] {
    ...
    std::array::from_fn(|index| {
        let row: adw::SwitchRow =
            gtk_widgets::ui_resource::object(builder, resource, &format!("content_{}", ids[index]));
...
pub(super) fn groups(
    ...
    enabled.connect_active_notify(move |row| {
        ...
        if !row.is_active() {
            shell
                .settings
                .set_app_setting("automatic backups", false, |s| &mut s.backup.enabled);
            return;
        }
            ...
            if let Some((password, _)) = options(&shell, false).await {
                ...
                match saved {
                    Ok(()) => {
                        shell
                            .settings
                            .update_app_settings("automatic backups", |settings| {
    ...
    encrypt.connect_active_notify(move |row| {
        ...
        if let Some(shell) = weak.upgrade() {
            shell
                .settings
                .set_app_setting("backup encryption", row.is_active(), |s| {
    ...
    weekday.connect_selected_notify(move |row| {
        if let Some(shell) = weak.upgrade() {
            shell
                .settings
                .set_app_setting("backup weekday", row.selected() as u8, |s| {
    ...
    hour.connect_selected_notify(move |row| {
        if let Some(shell) = weak.upgrade() {
            shell
                .settings
                .set_app_setting("backup hour", row.selected() as u8, |s| {
    ...
    choose_folder.connect_clicked(move |_| {
        ...
        glib::spawn_future_local(async move {
            if let Ok(selected) = chooser("folder_chooser")
                .select_folder_future(Some(&shell.window))
                .await
            {
                if shell
                    .settings
    ...
    if let Some(source) = &selected {
        let name = gtk_widgets::source_labels::configured_source_display_name(source);
    ...
    activity_export.connect_clicked(move |_| {
        if let Some(shell) = weak.upgrade() {
            ...
            (shell.effects)(crate::Effect::ExportActivity(format, source));
        }
    });
    let resource = crate::ui_resource::BACKUP_DIALOG_RESOURCE;
    let builder = gtk_widgets::ui_resource::builder(resource);
    let contents_group: adw::PreferencesGroup =
        gtk_widgets::ui_resource::object(&builder, resource, "contents_group");
        ...
        .enumerate()
    {
        ...
        row.connect_active_notify(move |row| {
            if let Some(shell) = weak.upgrade() {
                shell
                    .settings
                    .set_app_setting("backup contents", row.is_active(), |s| {
    ...
    retention_count.connect_value_changed(move |row| {
        if let Some(shell) = weak.upgrade() {
            shell
                .settings
                .set_app_setting("backup retention", row.value() as u32, |s| {
...
fn export_dialog(shell: &Rc<Preferences>) {
...
pub fn import_dialog(shell: &Rc<Preferences>) {
    let shell = Rc::clone(shell);
    glib::spawn_future_local(async move {
        ...
        let confirm: adw::AlertDialog =
            gtk_widgets::ui_resource::object(&builder, resource, "restore_confirm");
        ...
        if confirm.choose_future(Some(&shell.window)).await.as_str() != "restore" {
            return;
        }
            ...
            Ok(report) => {
                ...
                *shell.settings.current.borrow_mut() = shell.settings.persistence.load();
                ...
                if affected.settings {
                    ...
                    (shell.effects)(crate::Effect::LayoutChanged);
                    (shell.effects)(crate::Effect::PrivateModeChanged);
                    (shell.effects)(crate::Effect::MediaControlsChanged);
                    shell.player_ui.render_lyrics_panel();
                }
                if affected.settings || affected.playlists {
                    (shell.effects)(crate::Effect::SidebarChanged);
                }
                (shell.effects)(crate::Effect::ReloadCatalog);
                if affected.queue {
                ...
                if !report.warnings.is_empty() {
                    ...
                    shell.control_feedback.show_feedback_toast(tr_with("Backup restored with {entries} skipped playlist entries and {listens} skipped listens.",
...
async fn options(shell: &Preferences, importing: bool) -> Option<(Option<String>, BackupContents)> {
    let resource = crate::ui_resource::BACKUP_DIALOG_RESOURCE;
    let builder = gtk_widgets::ui_resource::builder(resource);
    gtk_widgets::objects!(builder, resource, {
    ...
    if options.choose_future(Some(&shell.window)).await.as_str() != "continue" {
        return None;
    }
    Some((
...
fn feedback(shell: &Preferences, result: Result<(), String>, success: &str) {
    match result {
        Ok(()) => shell.control_feedback.show_feedback_toast(tr(success)),
        Err(error) => shell
            .control_feedback
            .show_feedback_toast(tr_with("Backup failed: {error}", &[("error", &error)])),
    }
}
fn chooser(id: &str) -> gtk::FileDialog {
    let resource = crate::ui_resource::BACKUP_DIALOG_RESOURCE;
    gtk_widgets::ui_resource::object(&gtk_widgets::ui_resource::builder(resource), resource, id)
}
