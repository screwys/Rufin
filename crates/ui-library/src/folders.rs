            ...
            Err(error) => {
                ...
                (
                    ...
                    Rc::new(|_: &downloads::DownloadEvent| {})
                        as ui_shared::mounted_route::MountedDownloadChange,
                )
            }
        };
...
fn folder_page(
    ...
    ui_shared::layout::configure_fill_width_clip(&table_scroller, gtk::PolicyType::Automatic);
    ...
    let table_view = ui_shared::layout::width_allocation_owner(&table_view, move |width| {
...
fn folder_table(
    ...
    tracks: Rc<ui_shared::sparse_model::SparseRouteModel<String, FolderTableRow>>,
    path: Vec<FolderPathItem>,
    initial_width: i32,
    ...
    let width_fit = ui_shared::table_sizing::install_column_view_width_fit(
        &table,
        columns
            .iter()
...
fn folder_merged_column(shell: &Rc<CatalogUi>, path: Vec<FolderPathItem>) -> gtk::ColumnViewColumn {
    ...
    factory.connect_setup(move |_, item| {
        ...
        let cell = ui_shared::recycled_cells::RecycledMergedCell::new(
    ...
    connect_sparse_bind(&factory, move |item| {
        ...
        let Some(cell) = ui_shared::recycled_cells::list_cell::<
            ui_shared::recycled_cells::RecycledMergedCell,
        >(item) else {
            return;
        };
        let Some(icon) = cell
    ...
    factory.connect_unbind(move |_, item| {
        if let Some(cell) = item.downcast_ref::<gtk::ListItem>().and_then(
            ui_shared::recycled_cells::list_cell::<ui_shared::recycled_cells::RecycledMergedCell>,
        ) {
            clear_shell.artwork.clear_artwork_tile(&cell.cover());
    ...
    let column = gtk::ColumnViewColumn::new(
        Some(&tr(ui_shared::settings::library_field_title(
            LibraryField::Title,
        ))),
        Some(factory),
    );
...
fn folder_album_column(shell: &Rc<CatalogUi>, path: Vec<FolderPathItem>) -> gtk::ColumnViewColumn {
    let column = folder_text_column(
        shell,
        path,
        ui_shared::settings::library_field_title(LibraryField::Album),
        24,
        false,
        |row| match row {
        ...
        |row| match row {
            FolderTableRow::Folder(_) => None,
            FolderTableRow::Track(track) => Some(ui_shared::detail_links::DetailLinks::route(
                &track.album,
...
fn folder_year_column(shell: &Rc<CatalogUi>, path: Vec<FolderPathItem>) -> gtk::ColumnViewColumn {
    let column = folder_text_column(
        shell,
        path,
        ui_shared::settings::library_field_title(LibraryField::Year),
        8,
        true,
        |row| match row {
            FolderTableRow::Folder(_) => String::new(),
            FolderTableRow::Track(track) => {
                ui_shared::library_fields::track_field(track, LibraryField::Year)
            }
        },
        |_| None,
    );
...
fn folder_duration_column(
    ...
    let column = folder_text_column(
        ...
        |row| match row {
            FolderTableRow::Folder(_) => String::new(),
            FolderTableRow::Track(track) => {
                ui_shared::format_duration((track.duration_millis.max(0) / 1_000) as u32)
            }
        },
        |_| None,
    );
...
fn folder_text_column(
    ...
    links: impl Fn(&FolderTableRow) -> Option<ui_shared::detail_links::DetailLinks> + 'static,
) -> gtk::ColumnViewColumn {
    let factory = gtk::SignalListItemFactory::new();
    let setup_shell = Rc::clone(shell);
    factory.connect_setup(move |_, item| {
    ...
    connect_sparse_bind(&factory, move |item| {
        ...
        let Some(cell) = ui_shared::recycled_cells::list_cell::<
            ui_shared::recycled_cells::RecycledTextCell,
        >(item) else {
            return;
        };
        let text = value(&row);
        cell.bind_links(
            links(&row).unwrap_or_else(|| ui_shared::detail_links::DetailLinks::text(&text)),
        );
    });
    factory.connect_unbind(|_, item| {
        let Some(cell) = item.downcast_ref::<gtk::ListItem>().and_then(
            ui_shared::recycled_cells::list_cell::<ui_shared::recycled_cells::RecycledTextCell>,
        ) else {
            return;
        };
        cell.clear();
...
mod tests {
    ...
    fn folder_keys_use_shared_ready_row_extraction() {
        use gtk::prelude::*;
        use ui_shared::sparse_model::{SparseObjectModel, item_at};
