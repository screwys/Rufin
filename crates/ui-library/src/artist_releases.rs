...
use ui_shared::library_fields::COLLECTION_GRID_MIN_CARD_WIDTH;
use ui_shared::sparse_model::{SparseObjectItem, SparseRouteModel, connect_sparse_bind};
...
impl ArtistAlbumProjection {
    fn new(
        ...
        let heading = ui_shared::localization::localized_label(title);
...
fn artist_route_list(
    ...
    connect_sparse_bind(&factory, move |item| {
        ...
        let Some(row) = ui_shared::sparse_model::item_at_from_item::<ArtistRouteRow>(item) else {
            return;
        };
