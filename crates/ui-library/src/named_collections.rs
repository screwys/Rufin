...
use ui_shared::sparse_model::item_at_from_item;
...
use ui_shared::artwork::CoverGroupProjection;
...
use ui_shared::detail_links::DetailLinks;
...
impl<T: NamedCollectionRow> NamedCollectionGridCell<T> {
    fn new(shell: Rc<CatalogUi>, fields: &[LibraryField]) -> Self {
        ...
        let cover = shell.artwork.elastic_cover_group_projection_for_artwork(
            &[],
            ui_shared::library_fields::COLLECTION_GRID_MAX_CARD_WIDTH,
        );
    ...
    match field {
        ...
        LibraryField::Title | LibraryField::TitleMerged => {
            ...
            gtk::ColumnViewColumn::new(
                Some(ui_shared::settings::library_field_title(field)),
                Some(factory),
            )
        }
        _ => {
