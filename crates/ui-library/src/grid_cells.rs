...
use ui_shared::detail_links::{DetailLinkBinding, DetailLinks, album_artist_links};
...
use ui_shared::sparse_model::{bind_sparse_item, unbind_sparse_item};
...
pub fn install_grid_context<T: Clone + 'static>(
    ...
    menu.connect_clicked(move |_| {
        ...
        menu_present(
            ...
            &shell,
            row,
            ui_shared::cover_controls::elastic_cover_context_point(&target),
        );
    });
...
pub struct TrackGridCell<T: ui_shared::library_fields::TrackPresentation = library::TrackRow> {
...
impl<T: ui_shared::library_fields::TrackPresentation> TrackGridCell<T> {
    pub fn new(
        shell: &Rc<CatalogUi>,
        fields: &[LibraryField],
        play: Rc<dyn Fn(u32)>,
...
impl<T: ui_shared::library_fields::TrackPresentation> ReusableCollectionGridCell<T>
    for TrackGridCell<T>
{
    fn widget(&self) -> gtk::Widget {
        self.body.widget()
    ...
    pub fn enable_card_activation(&self) {
        ...
        click.connect_pressed(move |gesture, presses, _, _| {
                ...
                && let Some(uri) = uri
            {
                ...
                shell.navigate(ui_shared::route::Route::AlbumDetail(uri));
            }
        });
