...
fn album_lead_meta(
    ...
    if let Ok(label) = artist
        ...
        .ok_or(())
    {
        ui_shared::detail_links::DetailLinkBinding::new(&label, shell.route_navigation())
            .bind(ui_shared::detail_links::album_artist_links(album));
    }
    meta.append(&artist);
    ...
    facts.push(ui_shared::format_duration_units(
...
fn album_track_field_widths(
    ...
    if title_count == 0 || fixed.saturating_add(title_count) > available {
        let widths = ui_shared::table_sizing::fitted_column_widths(
