...
#[test]
fn controller_http_api_and_browser_share_application_operations() {
...
async fn controller_status(
    status: &mut tokio::sync::watch::Receiver<web::ControllerStatus>,
    predicate: impl Fn(&web::ControllerStatus) -> bool,
) -> web::ControllerStatus {
...
async fn exercise_http_api(
    ...
    let server = tokio::spawn(web::serve(listener, products.clone(), token.into()));
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(5))
        .build()
        .unwrap();
...
async fn exercise_browser_restart(products: rufin_core::runtime::ProductHandles) {
    ...
    for (token, expected) in [
        ...
        let server = tokio::spawn(web::serve(socket, products.clone(), token.into()));
        let client = reqwest::Client::builder()
            .redirect(reqwest::redirect::Policy::none())
            .timeout(Duration::from_secs(5))
            .build()
