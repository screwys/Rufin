...
async fn selected_uris(
    ...
    let target = match selection {
        MediaSelection::Album { id } => rufin_core::playback::PlaybackTarget::AlbumKey(id),
        MediaSelection::Artist { id, album_artists } => {
            rufin_core::playback::PlaybackTarget::ArtistKey(id, album_artists)
        }
        MediaSelection::Playlist { id } => rufin_core::playback::PlaybackTarget::Playlist(id),
        MediaSelection::Genre { id } => rufin_core::playback::PlaybackTarget::Genre(id),
        MediaSelection::SmartPlaylist { id } => {
            rufin_core::playback::PlaybackTarget::SmartPlaylist(id)
        }
    };
...
async fn source_file_settings(
    ...
    let enabled = completion(rufin_core::playlist_files::source_auto_save(
        &products.source,
...
async fn source_file_update(
    ...
    completion(rufin_core::playlist_files::set_source_auto_save(
        &products.source,
        input.source,
        input.auto_save,
    ))
...
async fn public(
    ...
    let public = completion(rufin_core::playlists::playlist_public(
        &products.source,
        key(&parameters, "id")?,
    ))
    .await?;
...
async fn file_settings(
    ...
    let settings = completion(rufin_core::playlist_files::settings(
        &products.source,
        key(&parameters, "id")?,
    ))
    .await?;
...
async fn file_update(
    ...
    let input: Input = body(request).await?;
    completion(rufin_core::playlist_files::update(
        &products.source,
        input.id,
        input.name,
        input.auto_refresh,
...
async fn file_action(
    ...
    let result = match input.action.as_str() {
        "save" => rufin_core::playlist_files::save(owner, input.id, input.force),
        "reload" => rufin_core::playlist_files::reload(owner, input.id, input.force),
        "unlink" => rufin_core::playlist_files::unlink(owner, input.id),
        "delete" => rufin_core::playlist_files::delete_file(owner, input.id),
        "rename" => rufin_core::playlist_files::rename_file(
            owner,
            input.id,
        ...
        "link" => {
            ...
            if let Some(source) = input.source {
                rufin_core::playlist_files::link_source(owner, input.id, source, path, input.force)
            } else {
                rufin_core::playlist_files::link_local(owner, input.id, path.into(), input.force)
            }
        }
    ...
    match result.recv().await.map_err(internal)? {
        Ok(()) => {}
        Err(error) if rufin_core::playlist_files::is_conflict(&error) => {
...
async fn import_file(
    ...
    for path in input.paths {
        let report = if let Some(source) = &input.source {
            completion(rufin_core::playlists::import_source_playlist(
                &products.source,
                source.clone(),
                path,
                !input.copy,
            ...
            completion(rufin_core::playlists::import_playlist(
                &products.source,
                path.into(),
                None,
                !input.copy,
...
async fn export_file(
    ...
    let input: Input = body(request).await?;
    let target = rufin_core::runtime::source::PlaylistExport::Playlist(input.id);
    let receiver = rufin_core::playlists::export_playlist(
        &products.source,
        input.source,
        input.path.into(),
        target,
...
async fn create(
    ...
    .await?;
    let object_id = completion(rufin_core::playlists::create_playlist(
        &products.source,
        input.source,
        input.name,
        uris,
    ...
    let id = if let Some(object) = object_id.as_deref() {
        ...
        let playlist_id = object.to_owned();
        settings_error = tokio::task::spawn_blocking(move || {
            settings.remember_created_playlist(playlist_source, playlist_id, input.current)
        })
        .await
...
async fn rename(
    ...
    let input: Input = body(request).await?;
    let changed = completion(rufin_core::playlists::update_playlist(
        &products.source,
        input.id,
        input.name,
        input.public,
...
async fn delete(
    ...
    let changed = completion(rufin_core::playlists::delete_playlist(
        &products.source,
        key(parameters, "id")?,
    ))
    .await?;
...
async fn add_entries(
    ...
    .await?;
    let added = completion(rufin_core::playlists::add_playlist_tracks(
        &products.source,
        input.id,
        uris,
...
async fn remove_entries(
    ...
    let input: Input = body(request).await?;
    let changed = completion(rufin_core::playlists::remove_playlist_entries(
        &products.source,
        input.id,
        input.entries,
    ))
...
async fn move_entry(
    ...
    let input: Input = body(request).await?;
    let changed = completion(rufin_core::playlists::move_playlist_entry(
        &products.source,
        input.id,
        input.entry,
        input.position,
