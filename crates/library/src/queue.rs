...
use crate::{
    AlbumKey, Database, LibraryError, LibraryResult, QueueOccurrenceKey, ReadCancellation,
    SourceKey, TrackKey,
};
...
impl Database {
    ...
    pub async fn persist_compact_queue(
        &self,
        source: SourceKey,
        ...
        drop(writer);
        self.restore_queue(source).await
    }
...
async fn load_queue_media(
    ...
    query.push(") SELECT occurrence.queue_occurrence_key occurrence_key,occurrence.track_key,occurrence.track_object_id,COALESCE(track.title,occurrence.fallback_title,'') title,COALESCE(track.display_artist,occurrence.fallback_artist,'') artist,COALESCE(track.display_album,occurrence.fallback_album,'') album,COALESCE(album.display_artist,occurrence.fallback_album_display_artist) album_display_artist,track.album_key,COALESCE(album.object_id,occurrence.fallback_album_object_id) album_object_id,COALESCE((SELECT artist.object_id FROM track_artists credit JOIN artists artist USING(artist_key) WHERE credit.track_key=track.track_key ORDER BY credit.position LIMIT 1),(SELECT artist.object_id FROM album_artists credit JOIN artists artist USING(artist_key) WHERE credit.album_key=track.album_key ORDER BY credit.position LIMIT 1),occurrence.fallback_primary_artist_object_id) primary_artist_object_id,COALESCE((SELECT artist.musicbrainz_artist_id FROM track_artists credit JOIN artists artist USING(artist_key) WHERE credit.track_key=track.track_key ORDER BY credit.position LIMIT 1),(SELECT artist.musicbrainz_artist_id FROM album_artists credit JOIN artists artist USING(artist_key) WHERE credit.album_key=track.album_key ORDER BY credit.position LIMIT 1),occurrence.fallback_primary_artist_musicbrainz_id) primary_artist_musicbrainz_id,COALESCE(access.media_uri,track.media_uri,occurrence.fallback_media_uri) media_uri,COALESCE(track.artwork_binding,album.artwork_binding,occurrence.fallback_artwork_binding) artwork_binding,COALESCE(track.duration_millis,occurrence.fallback_duration_millis) duration_millis,COALESCE(track.disc_number,occurrence.fallback_disc_number) disc_number,COALESCE(track.track_number,occurrence.fallback_track_number) track_number,COALESCE(track.year,occurrence.fallback_year) year,COALESCE(track.release_date,occurrence.fallback_release_date) release_date,COALESCE(COALESCE(track.user_favorite,track.source_favorite),occurrence.fallback_favorite) favorite,COALESCE(track.source_format,occurrence.fallback_source_format) source_format,COALESCE(track.musicbrainz_recording_id,occurrence.fallback_musicbrainz_recording_id) musicbrainz_recording_id,COALESCE(track.musicbrainz_release_track_id,occurrence.fallback_musicbrainz_release_track_id) musicbrainz_release_track_id,COALESCE(album.musicbrainz_release_id,occurrence.fallback_musicbrainz_album_id) musicbrainz_album_id,COALESCE(album.musicbrainz_release_group_id,occurrence.fallback_musicbrainz_release_group_id) musicbrainz_release_group_id,COALESCE(track.cue_path,occurrence.fallback_cue_path) cue_path,COALESCE(track.cue_start_millis,occurrence.fallback_cue_start_millis) cue_start_millis,COALESCE(track.cue_end_millis,occurrence.fallback_cue_end_millis) cue_end_millis FROM requested JOIN queue_occurrences occurrence ON occurrence.queue_occurrence_key=requested.occurrence_key LEFT JOIN tracks track USING(track_key) LEFT JOIN albums album ON album.album_key=track.album_key LEFT JOIN local_access_files access ON access.source_key=occurrence.source_key AND access.track_object_id=occurrence.track_object_id WHERE occurrence.source_key=").push_bind(source).push(" ORDER BY requested.ordinal");
    let rows = query
