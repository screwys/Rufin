...
pub struct LyricsState {
    ...
    pub dictionary_toast: RefCell<Option<adw::Toast>>,
}

pub struct SelectedLyricsState {
    pub projection: RefCell<CurrentLyrics>,
    pub offset_millis: Cell<i64>,
    pub timing_source: RefCell<Option<glib::SourceId>>,
    pub timing: RefCell<LyricsTiming>,
...
impl PlayerUi {
    pub fn right_lyrics_surface_visible(&self) -> bool {
    ...
    pub fn visible_lyrics(&self) -> Option<Arc<LyricsDocument>> {
    ...
    pub fn visible_lyrics_are_instrumental(&self) -> bool {
        let current_media = current_playback_media_id(self.selected_playback().as_deref());
    ...
    pub fn visible_lyrics_pronunciation(&self) -> Option<Arc<LyricsDocument>> {
        let current_media = current_playback_media_id(self.selected_playback().as_deref());
        let lyrics = self.selected_lyrics()?;
        match &*lyrics.projection.borrow() {
            CurrentLyrics::Ready {
    ...
    pub fn current_lyrics_loading(&self) -> bool {
        let current_media = current_playback_media_id(self.selected_playback().as_deref());
        let Some(lyrics) = self.selected_lyrics() else {
            return false;
        };
    ...
    pub fn apply_current_lyrics(self: &Rc<Self>, projection: CurrentLyrics) {
        let Some(lyrics) = self.selected_lyrics() else {
            return;
        };
        ...
        let changed = lyrics.apply_projection(projection);
    ...
    pub fn refocus_current_lyrics_highlight(&self) {
    ...
    pub fn request_initial_lyrics_if_needed(&self) {
        self.request_auto_lyrics_if_needed();
    }

    pub fn request_auto_lyrics_if_needed(&self) {
        ...
        self.lyrics_service.load(media_id);
    }

    pub fn suppress_auto_lyrics_for_current(self: &Rc<Self>) {
        let Some(media_id) = current_playback_media_id(self.selected_playback().as_deref()) else {
            return;
        };
        ...
        self.lyrics_service.clear_fetched(media_id);
        self.settings
            .update_app_settings("lyrics auto-search setting", |settings| {
    ...
    pub fn adjust_lyrics_offset(self: &Rc<Self>, delta_millis: i64) {
        let Some(lyrics) = self.selected_lyrics() else {
            return;
        };
    ...
    pub fn set_lyrics_offset_from_text(self: &Rc<Self>, value: &str) {
    ...
    pub fn apply_lyrics_offset_from_text(self: &Rc<Self>, value: &str) {
        let Some(offset_millis) = parse_lyrics_offset_millis(value) else {
            return;
        };
        let Some(lyrics) = self.selected_lyrics() else {
    ...
    pub fn current_position_millis(&self) -> u64 {
        self.selected_playback()
            .as_deref()
            .map_or(0, |player| player.transport.position_millis)
    }

    pub fn seek_to_lyrics_position(self: &Rc<Self>, position_millis: u64) {
        let Some(lyrics) = self.selected_lyrics() else {
            return;
        };
...
impl SelectedLyricsState {
    pub(super) fn apply_projection(&self, projection: CurrentLyrics) -> bool {
        let document_changed = match (&*self.projection.borrow(), &projection) {
            (
                CurrentLyrics::Ready {
                    content:
        ...
        let media_id = match &projection {
            CurrentLyrics::Ready { media_id, .. } | CurrentLyrics::Loading { media_id } => {
                Some(media_id)
            }
            CurrentLyrics::Cleared => None,
        };
        let media_changed = match &*self.projection.borrow() {
        ...
        if document_changed || media_changed {
            self.right_pane.restart_follow_tracking();
            self.fullscreen_pane.restart_follow_tracking();
            self.offset_millis.set(0);
        }
        match &projection {
            CurrentLyrics::Ready {
                content: Some(CurrentLyricsContent::Document { document, .. }),
                ...
                *self.timing.borrow_mut() = LyricsTiming::new(&document.lines);
            }
            CurrentLyrics::Ready {
                content: Some(CurrentLyricsContent::Document { .. }),
                ..
            } => {}
            _ => *self.timing.borrow_mut() = LyricsTiming::default(),
        }
        *self.projection.borrow_mut() = projection;
        document_changed || media_changed
    }
}
