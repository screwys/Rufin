...
pub struct RightPanelWidgets {
    ...
    pub queue_lyrics_split: gtk::Paned,
    pub lyrics_surface: gtk::Box,
...
use crate::bottom::BOTTOM_PLAYER_HEIGHT;
use localization::tr;
use rufin_core::settings::layout::MIN_RESTORED_WINDOW_HEIGHT;
use std::rc::Rc;
const QUEUE_LYRICS_DEFAULT_LYRICS_HEIGHT: i32 = 300;
const QUEUE_LYRICS_HANDLE_HEIGHT: f64 = 10.0;
impl crate::PlayerUi {
    pub fn save_queue_lyrics_height(&self) {
        ...
        self.settings
            .set_app_setting("queue lyrics height", Some(height), |settings| {
                &mut settings.queue_lyrics_height
            });
    }
    pub fn remember_queue_lyrics_open_position(&self) {
...
pub fn connect_queue_lyrics_split(shell: &Rc<crate::PlayerUi>) {
...
fn queue_lyrics_restore_available_height(shell: &crate::PlayerUi) -> i32 {
    let split_height = shell.right_panel.queue_lyrics_split.height();
    if split_height > 1 {
        return split_height;
    }
    let window_height = shell.window.upgrade().map_or(0, |window| window.height());
