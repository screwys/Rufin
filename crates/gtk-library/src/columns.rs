use gtk_widgets::media_drag::{MediaDragSource, install_media_drag_source};
use gtk_widgets::sparse_model::{item_at_from_item, object_item};
...
use gtk_widgets::localization::localized_column;
...
use gtk_widgets::detail_links::{DetailLinks, album_artist_links};
...
pub fn album_column(
    ...
    connect_sparse_bind(&factory, |item| {
        ...
        if let Some(cell) = list_cell::<AlbumTableCell>(item) {
            let row = item
                .item()
                .and_downcast::<gtk_widgets::sparse_model::SparseObjectItem>()
    ...
    if field == LibraryField::Tools {
        return gtk_widgets::recycled_cells::row_actions_column(&factory);
    }
...
pub(super) enum AlbumCellView {
    ...
    Tools(gtk_widgets::recycled_cells::RowActions),
}
    ...
    pub fn new(
        ...
        let view = match field {
            ...
            LibraryField::Tools => {
                ...
                AlbumCellView::Tools(gtk_widgets::recycled_cells::RowActions::with_favorite(
                    &button,
                ))
            }
            _ => {
...
pub fn song_column_for_key<T: gtk_widgets::library_fields::TrackPresentation>(
    shell: &Rc<CatalogUi>,
    key: LibraryListKey,
    field: LibraryField,
    ...
    match field {
        ...
        LibraryField::AlbumArtist => mapped_track_link_column(
            shell,
            gtk_widgets::settings::library_field_title(LibraryField::AlbumArtist),
            width,
            |item: &T| Some(item.media_uri().to_string()),
    ...
    F: Fn(&T) -> String + 'static,
{
    ...
    let column = localized_column(gtk_widgets::settings::library_field_title(field), &factory);
    column.set_fixed_width(width);
    column
}
...
fn apply_track_row_playing(cell: &gtk::Widget, playing: bool, paused: bool) {
    gtk_widgets::recycled_cells::set_track_playing(cell, playing, paused);
}

pub use gtk_widgets::recycled_cells::{set_track_row_index_text, track_row_index_cell};
...
fn favorite_cell_button(item: &gtk::ListItem) -> Option<gtk::Button> {
    gtk_widgets::recycled_cells::row_favorite_button(item)
}
    ...
    FavoriteValue: Fn(&T) -> Option<(String, bool)> + 'static,
{
    ...
    connect_sparse_bind(&factory, move |item| {
        ...
        let actions =
            gtk_widgets::recycled_cells::list_cell::<gtk_widgets::recycled_cells::RowActions>(item);
