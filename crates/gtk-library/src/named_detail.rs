...
use gtk_media_menus::media_menus::{present_genre_context_menu, present_mood_context_menu};

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NamedDetailId {
    Genre(GenreKey),
    Mood(MoodKey),
}

#[derive(Clone)]
pub enum NamedDetailSummary {
    Genre(GenreRow),
    Mood(MoodRow),
}

pub struct NamedOrderLane {
    generation: Cell<u64>,
...
impl NamedOrderLane {
    ...
    pub fn begin(&self) -> (u64, ReadCancellation) {
...
impl CatalogUi {
    pub fn named_detail_view(
        ...
        let downloads = crate::collection_download_change(move |identity, downloaded| {
            match &mut *download_summary.borrow_mut() {
                ...
                NamedDetailSummary::Mood(row) if identity == format!("mood:{}", row.mood_key) => {
                    row.downloaded_count = if downloaded { row.track_count } else { 0 };
                }
                _ => {}
            }
...
fn named_context_menu(
    ...
    Some(Rc::new(
        move |target: &gtk::Widget, position, play| match &*current.borrow() {
            NamedDetailSummary::Genre(summary) => present_genre_context_menu(
                target,
                &shell.media_menus,
                summary.clone(),
                Some(play),
            ...
            NamedDetailSummary::Mood(summary) => present_mood_context_menu(
                target,
                &shell.media_menus,
                summary.clone(),
                Some(play),
...
fn named_kind_controls(shell: &Rc<CatalogUi>, id: NamedDetailId) -> Vec<gtk::Widget> {
    let mut controls = Vec::new();
    if let NamedDetailId::Genre(key) = id {
        let radio = detail_radio_button();
        let controller = shell.radio.clone();
