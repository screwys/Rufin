...
use gtk_widgets::layout::{configure_fill_width_clip, width_allocation_owner};
use gtk_widgets::localization::{bind_label_text_with, localized_label};
...
use gtk_widgets::library_fields::{COLLECTION_GRID_CARD_MARGIN, track_field};
        ...
        for (block, kind, rows) in [
                ...
                && let Some(slot) = self.slots.get(&block)
            {
                let view = shell.home_section_view(
                    &tr(gtk_widgets::settings::home_block_title(block)),
                    items,
                    Some(kind),
                );
...
impl CatalogUi {
    ...
    fn home_showcase(
        ...
        for button in [favorite, hover_favorite] {
            ...
            self.register_dynamic_favorite_button(
                Rc::new(move || Some(gtk_widgets::favorites::album_favorite_key(&favorite_key))),
                &button,
            );
    ...
    fn home_genres(self: &Rc<Self>, genres: &[HomeGenreRow]) -> Option<gtk::Widget> {
        ...
        let section = gtk::Box::new(gtk::Orientation::Vertical, 10);
        section.set_hexpand(true);
        let heading = localized_label(gtk_widgets::settings::home_block_title(
