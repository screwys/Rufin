...
use gtk::{gio, glib};
use gtk_media_menus::media_menus::{present_album_context_menu, present_artist_context_menu};
...
use gtk_widgets::media_drag::{MediaDragSource, install_compact_media_drag_source};
use gtk_widgets::mounted_route::{MountedRouteItemNavigation, item_navigation_entry_position};
use gtk_widgets::sparse_model::{item_at, item_at_from_item};
...
use super::cards;
use super::collections::collection_grid_field_label;
use rufin_core::playback::PlaybackTarget;

use crate::track_selection::TrackSelection;
use gtk_widgets::detail_links::{DetailLinkBinding, DetailLinks, album_artist_links};
use gtk_widgets::library_fields::{
    COLLECTION_GRID_CARD_MARGIN, COLLECTION_GRID_MAX_CARD_WIDTH, COLLECTION_GRID_MIN_CARD_WIDTH,
    album_field, artist_field, opaque_artwork,
};
use gtk_widgets::sparse_model::{bind_sparse_item, unbind_sparse_item};
...
pub fn install_grid_context<T: Clone + 'static>(
    shell: &Rc<CatalogUi>,
    current: &Rc<RefCell<Option<T>>>,
    menu: &gtk::Button,
    overlay: &gtk::Overlay,
    card: &impl IsA<gtk::Widget>,
    present: impl Fn(&gtk::Widget, &Rc<CatalogUi>, T, Option<(f64, f64)>) + 'static,
) {
    ...
    menu.connect_clicked(move |_| {
        ...
        menu_present(
            ...
            &shell,
            row,
            gtk_widgets::cover_controls::elastic_cover_context_point(&target),
        );
    });
...
pub fn install_grid_play<T: 'static>(
    shell: &Rc<CatalogUi>,
    current: &Rc<RefCell<Option<T>>>,
    buttons: [gtk::Button; 3],
    play: impl Fn(&Rc<CatalogUi>, &T, QueuePlacement) + 'static,
) {
...
pub trait ReusableCollectionGridCell<T>: 'static {
...
#[derive(Clone)]
pub struct FixedPageCollectionRow {
    row: gtk::FlowBox,
...
pub fn fixed_page_collection_row<T, Make, Activate>(
    model: gio::ListStore,
    columns: usize,
    make_widget: Make,
    activate: Activate,
...
impl CollectionGridProjection {
    ...
    pub fn fit_allocation(&self, width: i32) {
        self.cache_bound.fit_allocation(width);
    }

    pub fn navigate(&self, direction: gtk::DirectionType) -> glib::Propagation {
        (self.navigation)(direction)
    }
}

pub fn collection_grid<T, Cell, Make, Activate, M>(
    model: M,
    fields: &[LibraryField],
    make_cell: Make,
    activate: Activate,
...
pub fn collection_grid_with_demand<T, Cell, Make, Activate, Demand, M>(
    model: M,
    fields: &[LibraryField],
    make_cell: Make,
    activate: Activate,
...
pub fn collection_grid_with_selection_and_demand<T, Cell, Make, Activate, Demand, M>(
    model: M,
    ...
    M: IsA<gio::ListModel> + Clone + 'static,
{
    let selection = selection.unwrap_or_else(|| {
        if model
            .as_ref()
            .is::<gtk_widgets::sparse_model::SparseObjectModel>()
        {
            gtk_widgets::selection::PositionSelectionModel::new(model.clone()).upcast()
        } else {
...
fn play_one_media_uri(shell: &CatalogUi, media_uri: String, placement: QueuePlacement) {
...
pub struct TrackGridCell<T: gtk_widgets::library_fields::TrackPresentation = library::TrackRow> {
...
impl<T: gtk_widgets::library_fields::TrackPresentation> TrackGridCell<T> {
    pub fn new(
        shell: &Rc<CatalogUi>,
        fields: &[LibraryField],
        play: Rc<dyn Fn(u32)>,
        ...
        install_compact_media_drag_source(&body.card, &drag_artwork, move || {
            ...
            let source = match selection.as_ref() {
                ...
                None => MediaDragSource::capture_target(
                    drag_shell.upgrade()?.selected_library().as_deref(),
                    PlaybackTarget::Track(track.media_uri().to_string()),
                ),
            };
            Some((source, track.title().to_string()))
        });
        body.set_download_badge(shell.downloads.download_badge(false));
        Self {
            body,
...
impl<T: gtk_widgets::library_fields::TrackPresentation> ReusableCollectionGridCell<T>
    for TrackGridCell<T>
{
    fn widget(&self) -> gtk::Widget {
        self.body.widget()
    ...
    fn clear(&self) {
        if let Some(shell) = self.shell.upgrade() {
            shell.artwork.clear_artwork_tile(&self.cover);
    ...
    pub fn enable_card_activation(&self) {
        ...
        click.connect_pressed(move |gesture, presses, _, _| {
                ...
                && let Some(uri) = uri
            {
                ...
                shell.navigate(gtk_widgets::route::Route::AlbumDetail(uri));
            }
        });
        self.body.card.add_controller(click);
    }

    pub fn new(shell: &Rc<CatalogUi>, fields: &[LibraryField], context: Option<String>) -> Self {
        ...
        body.set_download_badge(shell.downloads.download_badge(true));
        Self {
            body,
            shell: Rc::downgrade(shell),
            cover,
...
impl MediaPlayingBinding {
    ...
    pub fn refresh(&self, current: Option<&gtk_widgets::mounted_route::RouteCurrentTrack>) {
        ...
        gtk_widgets::recycled_cells::set_track_playing(
            &widget,
            playing,
...
impl CollectionGridCardCell {
    ...
    pub fn new(shell: &Rc<CatalogUi>, fields: &[LibraryField], cover: gtk::Widget) -> Self {
        ...
        gtk_widgets::recycled_cells::install_playing_indicator(&title, &title_row);
    ...
    pub fn set_download_badge(&self, badge: gtk::Image) {
    ...
    pub fn set_download_target(&self, shell: &Rc<CatalogUi>, downloaded: bool) {
        if let Some(badge) = self.downloaded.borrow().as_ref() {
            shell.downloads.bind_download_badge(badge, downloaded);
        }
    }
    ...
    pub fn bind(&self, title: &str, mut field_value: impl FnMut(LibraryField) -> DetailLinks) {
    ...
    pub fn clear(&self, shell: &Rc<CatalogUi>) {
        ...
        gtk_widgets::recycled_cells::set_track_playing(self.title.upcast_ref(), false, false);
        self.title.set_text("");
    ...
    pub fn replace_fields(&self, shell: &Rc<CatalogUi>, fields: &[LibraryField]) {
        if self
            .fields
            .borrow()
            .iter()
...
impl CollectionGridFieldCell {
    fn new(shell: &Rc<CatalogUi>, field: LibraryField) -> Self {
        let (widget, label) = collection_grid_field_label("", field);
        connect_label_tooltip(&label);
        let links = DetailLinkBinding::new(&label, shell.route_navigation());
        Self {
            field,
            widget,
            links,
