...
use gtk_widgets::layout::{AllocationOwner, allocation_owner};
...
use gtk_widgets::library_fields::{
    column_width, compact_header_column_width, grid_label_with_label,
};
use gtk_widgets::route::Route;
use gtk_widgets::table_sizing::{ColumnViewWidthFit, install_column_view_width_fit};

struct TrackTablePlayingStateInner<T: TrackPresentation> {
    model: TrackCollectionModel<T>,
    handler_model: gtk_widgets::sparse_model::SparseObjectModel,
...
fn matching_context_rank(
    context: Option<&gtk_widgets::mounted_route::RouteCurrentTrackContext>,
    base: &str,
    visible: &str,
) -> Option<usize> {
    context
...
pub fn artist_collection_projection(
    shell: &Rc<CatalogUi>,
    sparse: Rc<gtk_widgets::sparse_model::SparseRouteModel<ArtistKey, ArtistRow>>,
    key: LibraryListKey,
) -> LibraryCollectionProjection {
    let settings = shell.settings.current.borrow().library_list(key);
    let shell = Rc::clone(shell);
...
pub fn collection_column_width(field: LibraryField) -> i32 {
    match field {
        LibraryField::AlbumCount | LibraryField::SongCount => {
            compact_header_column_width(gtk_widgets::settings::library_field_title(field), 96)
        }
        LibraryField::Duration => {
            compact_header_column_width(gtk_widgets::settings::library_field_title(field), 128)
        }
...
pub fn track_table<T: TrackPresentation>(
    ...
    if key == LibraryListKey::AlbumDetailTracks {
        ...
        factory.connect_setup(|_, header| {
            ...
            let label: gtk::Label = gtk_widgets::ui_resource::object(&builder, resource, "heading");
...
pub fn install_playlist_reorder(
    ...
    install_playlist_order_drop(
        ...
        Rc::new(move |source| {
            ...
            gtk_media_menus::media_menus::add_drag_to_playlist(&menus, playlist, source)
        }),
        Rc::new(move |dragged, target| {
...
fn install_playlist_order_drop(
    ...
    drop_media: Rc<dyn Fn(gtk_widgets::media_drag::MediaDragSource) -> bool>,
        ...
        for context_id in [base, visible] {
            assert_eq!(
                matching_context_rank(
                    Some(&gtk_widgets::mounted_route::RouteCurrentTrackContext {
