...
impl Shell {
    ...
    pub(crate) fn schedule_startup_route_reveal(self: &Rc<Self>) {
        ...
        self.startup.route_allocated.set(false);
        self.update_layout();
        self.artwork.begin_startup_cover_prime();
        ...
        let deadline = glib::timeout_add_local_once(
            ...
            move || {
                ...
                if shell.startup.route_revealed.get() {
                    shell.artwork.finish_startup_cover_prime_gate();
                    return;
                }
                let pending_covers = shell.artwork.startup_cover_prime_pending_count();
                if pending_covers > 0 {
                    warn!(
                        pending_covers,
    ...
    pub(crate) fn cancel_startup_route_reveal(&self) {
        ...
        self.artwork.finish_startup_cover_prime_gate();
    }
...
fn library_loading_status(
    operation: &rufin_core::runtime::source::SourceOperation,
    initial_launch: bool,
) -> Option<String> {
    if initial_launch {
...
mod tests {
    use super::*;
    use rufin_core::runtime::source::{SourceOperation, SourceProgress, SourceProgressStage};
