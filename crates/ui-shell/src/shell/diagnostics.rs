...
use ui_shared::layout::{large_popup_content_height, large_popup_content_width};
...
pub(super) fn present_diagnostics(shell: &Rc<Shell>) {
    ...
    let dialog: adw::Dialog =
        ui_shared::ui_resource::object(&builder, resource, "diagnostics_dialog");
    ...
    let debug: adw::SwitchRow = ui_shared::ui_resource::object(&builder, resource, "debug");
    debug.set_active(shell.diagnostics.debug_enabled());

    let log: gtk::TextView = ui_shared::ui_resource::object(&builder, resource, "log");
    log.update_property(&[gtk::accessible::Property::Label(&tr("Diagnostic log"))]);
    let scroller: gtk::ScrolledWindow =
        ui_shared::ui_resource::object(&builder, resource, "log_scroller");
    let copy: gtk::Button = ui_shared::ui_resource::object(&builder, resource, "copy");
    let save: gtk::Button = ui_shared::ui_resource::object(&builder, resource, "save");
    let status: gtk::Label = ui_shared::ui_resource::object(&builder, resource, "status");
    drop(builder);
