...
pub(super) fn build_normal_navigation(shell: &Rc<Shell>) {
    ...
    {
        let descriptor = ui_shared::route::sidebar_route_descriptor(entry.item);
...
fn update_native_route_selection(shell: &Shell, active_route: &Route, pin_is_selected: bool) {
    ...
    let selected_index = if pin_is_selected {
        ...
        items
            .clone()
            .position(|entry| {
                ui_shared::route::sidebar_route_descriptor(entry.item).root_route == *active_route
            })
...
fn update_button_navigation_selection(container: &gtk::Box, active_route: &Route) {
    ...
    while let Some(widget) = child {
        ...
        let descriptor = ui_shared::route::sidebar_route_descriptor(item);
    ...
    fn route(&self) -> Route {
        match self {
            ...
            Self::Artist(artist, album_artist) => {
                ui_library::artist_detail_route(artist.media_uri.clone(), *album_artist)
            }
...
fn sidebar_route_at_position_in(
    ...
    Some(ui_shared::route::sidebar_route_descriptor(item.item).root_route)
}
...
mod tests {
    ...
    fn descriptor_ids_preserve_sidebar_settings_serialization() {
        for item in SidebarRouteItem::all() {
            ...
            assert_eq!(
                value.as_str(),
                Some(ui_shared::route::sidebar_route_descriptor(item).stable_id)
            );
            assert_eq!(
    ...
    fn nested_routes_leave_root_sidebar_items_unselected() {
        ...
        for route in routes {
            assert!(SidebarRouteItem::all().into_iter().all(|item| {
                ui_shared::route::sidebar_route_descriptor(item).root_route != route
            }));
        }
    }
