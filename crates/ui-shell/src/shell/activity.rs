...
impl ActivityView {
    fn new(shell: &Rc<Shell>) -> Rc<Self> {
        ...
        let genre_rows = (0..10).map(|_| {
            ...
            let builder = ui_shared::ui_resource::builder(resource);
            ui_shared::objects!(builder, resource, { row: gtk::Box, name: gtk::Label, count: gtk::Label, bar: gtk::ProgressBar });
        ...
        let counts = [3, 5, 10];
        ui_shared::interactions::keep_parent_grab_for_dropdown(&result_count);
