...
impl Shell {
    ...
    pub(crate) fn fullscreen_player_visible(&self) -> bool {
        self.player_ui.views.fullscreen_player.visible.get()
    }
    ...
    fn apply_layout_allocation(self: &Rc<Self>, width: i32, height: i32) {
        ...
        if self.fullscreen_player_visible() {
            self.player_ui
                .apply_fullscreen_responsive_layout_for_size(width, height);
        }
    }
    ...
    fn apply_resolved_layout_for_allocation(self: &Rc<Self>, resolved: ResolvedLayout) {
        ...
        set_widget_visible(&self.player_ui.views.player_controls.root, app_active);

        if right_visibility_changed {
            self.player_ui.update_right_panel_button();
            self.player_ui.sync_visualizer_state();
            let lyrics_shell = Rc::clone(self);
            glib::idle_add_local_once(move || {
                if lyrics_shell.player_ui.right_lyrics_surface_visible() {
                    lyrics_shell.player_ui.sync_visible_lyrics_surfaces();
                } else {
                    lyrics_shell.player_ui.update_lyrics_highlight();
                }
            });
        }
    ...
    fn apply_fullscreen_topology(&self, app_active: bool) {
        let fullscreen = &self.player_ui.views.fullscreen_player;
        if !app_active {
...
fn connect_right_sidebar_resize(shell: &Rc<Shell>) {
    ...
    drag.connect_drag_update(move |_, offset_x, _| {
        ...
        if let Some(lyrics) = drag_shell.player_ui.selected_lyrics() {
