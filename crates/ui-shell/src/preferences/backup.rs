...
fn content_rows(builder: &gtk::Builder, mut selected: BackupContents) -> [adw::SwitchRow; 7] {
    ...
    std::array::from_fn(|index| {
        let row: adw::SwitchRow =
            ui_shared::ui_resource::object(builder, resource, &format!("content_{}", ids[index]));
...
pub(super) fn groups(
    ...
    if let Some(source) = &selected {
        let name = ui_shared::source_labels::configured_source_display_name(source);
    ...
    let contents_group: adw::PreferencesGroup =
        ui_shared::ui_resource::object(&builder, resource, "contents_group");
...
pub(crate) fn import_dialog(shell: &Rc<Shell>) {
    ...
    glib::spawn_future_local(async move {
        ...
        let confirm: adw::AlertDialog =
            ui_shared::ui_resource::object(&builder, resource, "restore_confirm");
...
fn chooser(id: &str) -> gtk::FileDialog {
    let resource = crate::ui_resource::BACKUP_DIALOG_RESOURCE;
    ui_shared::ui_resource::object(&ui_shared::ui_resource::builder(resource), resource, id)
}
