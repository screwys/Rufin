...
use ui_shared::controls::text_button;
...
fn setup_scaffold(
    ...
    source_icon.set_icon_name(Some(
        ui_shared::source_labels::source_kind_icon_name(registration.kind)
            .expect("registered source presentation"),
    ));
    source_title.set_label(&tr(ui_shared::source_labels::source_kind_title(
        registration.kind,
    )
    .expect("registered source presentation")));
...
fn source_choice_selector(shell: &Rc<Shell>, context: &SetupViewContext) -> gtk::Widget {
    ...
    for &presentation in source_presentations() {
        ...
        row.set_title(&tr(ui_shared::source_labels::source_kind_title(
            presentation.kind,
        )
        .expect("registered source presentation")));
        icon.set_icon_name(Some(
            ui_shared::source_labels::source_kind_icon_name(presentation.kind)
                .expect("registered source presentation"),
        ));
