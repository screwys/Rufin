use crate::runtime::BackupPreview;
use crate::{
    playback::PlaybackOwner,
    settings::{
        SettingsFile, SettingsOwner, StoredSettings, backup_password_key, backup_password_store,
...
#[serde(default)]
struct PlaylistPins {
    pins: Vec<crate::settings::SidebarPin>,
...
impl BackupOwner {
    ...
    async fn export_now(
        ...
        backup::write_backup(
            ...
            backup::BackupOptions {
                ...
                schedule_id: scheduled.then_some(stored.ui.backup.schedule.schedule_id.as_str()),
            },
        )
        .await
        .map_err(|e| e.to_string())?;
    ...
    async fn scheduled(&self) -> Result<(), String> {
        ...
        let passphrase = if settings.encrypt {
            ...
            tokio::task::spawn_blocking(move || {
                backup_password_store(&stored, &config_dir)
    ...
    async fn restore_now(
        ...
        self.source
            .restore_user_state(contents.settings || contents.saved_logins, || async {
                ...
                let result = backup::restore_backup(&self.database, &preview.staged, contents)
                    .await
                    .map_err(|e| e.to_string());
...
impl BackupOwner {
    pub fn default_directory(&self) -> std::path::PathBuf {
    ...
    pub fn export(
        &self,
        passphrase: Option<String>,
    ) -> Receiver<Result<tempfile::TempPath, String>> {
    ...
    pub fn save_schedule_password(&self, passphrase: String) -> Receiver<Result<(), String>> {
        let (send, receive) = async_channel::bounded(1);
        let owner = self.clone();
        self.runtime.spawn_blocking(move || {
            let result = if passphrase.is_empty() {
                ...
                backup_password_store(&owner.settings.load(), owner.settings.config_dir())
...
fn prune_scheduled_backups(
        ...
        .map_err(|e| e.to_string())?
    {
        ...
        if let Some(timestamp) =
            backup::scheduled_backup_timestamp(&name.to_string_lossy(), schedule_id)
        {
...
mod tests {
        ...
        for retention_count in [1, 2, 3, 6] {
            ...
            for timestamp in 1..=4 {
                fs::write(
                    directory
                        .path()
                        .join(backup::backup_filename(Some("mine"), timestamp)),
                    [],
                )
                .unwrap();
            }
            let foreign = backup::backup_filename(Some("other"), 1);
            let manual = backup::backup_filename(None, 1);
            let incomplete = format!(".{}.partial", backup::backup_filename(Some("mine"), 5));
    ...
    fn setup_restore_retains_destination_namespace_and_exact_source_credentials() {
        ...
        previous.ui.sidebar.pins = vec![crate::settings::SidebarPin::Playlist {
            source_id: None,
