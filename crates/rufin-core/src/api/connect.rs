...
async fn continuation(
    State(products): State<ProductHandles>,
) -> Result<axum::Json<Option<crate::connect::Device>>, Error> {
    products
        .connect
        .continuation_offer()
        .await
...
async fn status(State(products): State<ProductHandles>) -> axum::Json<crate::connect::Status> {
    axum::Json(products.connect.status())
}

async fn action(
    State(products): State<ProductHandles>,
    axum::Json(action): axum::Json<crate::connect::Action>,
) -> Result<axum::Json<crate::connect::Status>, Error> {
    products
        .connect
        .execute(action)
        .await
